<!-- Ce script correspond à la page sur laquelle les modules sont visibles, soit la page sur laquelle
on est redirigé une fois connecté. -->
<!Doctype html>
<html>
	<head>
		<title>Orce.</title>
	...
	<body>
		...
		<div id="top-nav">
			...
			<div id="plus">+</div></p>
			<p class="notifs"><div id="notif">-</div></p>
			<!-- Menu déroulant -->
			<ul class="menuderoulant">
				<a href="?parametres"><li>Paramètres du compte</li></a><a href="?resumeEnseignements"><li>Enseignements</li></a><a href="?deconnexion"><li id="deco">Déconnexion</li></a>
			</ul>

			<ul class="notifications">

			</ul>
			<!-- Affichage heures totales / points dans l'entête -->
		...
		<div id="container" class="container">
			<?php
				if(count($formations) > 0){
					...
					//Les paramètres de réglages (ajouter un module, ajouter un enseignant) ne sont accèssibles qu'aux administrateurs
						if($formation["estAdmin"]){
							?>
							<div class="formation-container">
							<div class="afficherReglages"><button>Réglages</buton></div>
							<div class="reglages">
							<span>
								<!--Ajouter un enseignantà venir rejoindre la formation -->
								...
								<div class="fenetreModale">
									...
									<div class="fenetre-body">
								      ...
								      <input type="text" name="professeur" placeholder="prenom.nom@email.fr" required><br/><br/>
							...
							<span>
								...
								<button type="button" class="boutonAjouter">Ajouter un administrateur</button>
								<div class="fenetreModale">
									<div class="fenetre-header">
										Ajouter un administrateur<a class="close"><svg aria-label="Close" class="octicon octicon-x js-menu-close" height="16" role="img" version="1.1" viewBox="0 0 12 16" width="12"><path fill-rule="evenodd" d="M7.48 8l3.75 3.75-1.48 1.48L6 9.48l-3.75 3.75-1.48-1.48L4.52 8 .77 4.25l1.48-1.48L6 6.52l3.75-3.75 1.48 1.48z"></path></svg></a>
									</div>
									<div class="fenetre-body">
										<form action = "index.php?ajoutSecretaireFormationSub" method="post">
								      <input type="text" name="secretaire" placeholder="prenom.nom@email.fr" required><br/><br/>
								      <input type="hidden" name="formation" value="<?php echo htmlspecialchars($formation["ID"]);?>">
							...
							<?php
							echo " <a href='?ajoutNiveau&formID=".$formation["ID"]."'><button style='display:none;' type='hidden'>Ajouter un niveau</button></a><br /></div></div>";
						}
						if(count($formation["niveaux"]) <= 0){
							echo "<div class='vide'><p class='errorMessage'>Aucun niveau pour cette formation.</p>";
							if($formation["estAdmin"] || $formation["estAdmin"]){
								echo "<a href='?ajoutNiveau'><button  type=\"hidden\">Ajouter un niveau</button></a>";
							}
							echo "</div>";
						}else{
				...
				<?php if($formation["estAdmin"] || $formation["estAdmin"]){ ?>
				<span>
					<!-- Ajouter un module à la formation -->
					...
					<div class="fenetreModale">
				        ...
				        ID du Module : <input type="text" name="ID" placeholder="ID" required></br></br>
				        Nom : <input type="text" name="nom" placeholder="Nom" required></br></br>

								<input type="hidden" name="UE_ID" value="<?php echo $ue['ID']; ?>"/>
				        Nombre d'heures TDs : <input type="number" name="nbHeuresTD" placeholder="TD" min="0" required></br></br>
				        Nombre d'heures TPs : <input type="number" name="nbHeuresTP" placeholder="TP" min="0" required></br></br>
				        Nombre d'heures CMs : <input type="number" name="nbHeuresCM" placeholder="CM" min="0" required></br></br>
				        Nombre d'heures DS : <input type="number" name="nbHeuresDS" placeholder="DS" min="0" required></br></br>
				        Numéro de la période de départ : <input type="number" name="periode" min="1" max="4" placeholder="1-4" required></br></br>
								Durée du module (semaines) : <input type="number" name="duree" min="0" max="54" required><br><br>
	        ...
	        <?php
	                ...
	                foreach ($ue["modules"] as $module) {
	                  ...
	                  ?>
	                   ...
	                   <tr>
	                        ...
	                        <?php echo $module["nom"];?>
												...
												<td>
													<?php if(count($module["voeux"])){ ?>
														<!-- Voir quels sont les professeurs affectés au module sélectionné -->
	                      ...
	                      <td>
													<?php
													//Méthode pour ajouter un voeux sur un module
		                          ...
		                          if(!$aDejaFormuleVoeu){
		                              ...
		                              <div class="fenetreModale" style="display:none;">
		                                <div class="fenetre-header">
																			Ajouter un voeu <a class="close"><svg aria-label="Close" class="octicon octicon-x js-menu-close" height="16" role="img" version="1.1" viewBox="0 0 12 16" width="12"><path fill-rule="evenodd" d="M7.48 8l3.75 3.75-1.48 1.48L6 9.48l-3.75 3.75-1.48-1.48L4.52 8 .77 4.25l1.48-1.48L6 6.52l3.75-3.75 1.48 1.48z"></path></svg></a>
																		</div>
		                                <div class="fenetre-body">
		                                  <h4><?php echo $module["ID"]." - ".$module["nom"]; ?></h4>
		                              ...
		                              <?php
												...
												//Si l'utilisateur est administrateur il peut modifier le module
													if($formation["estAdmin"] || $formation["estAdmin"]){
														?>
															<td>
																...
																<div class="fenetreModale">
																	<div class="fenetre-header">
																		Modifier ce module <a class="close"><svg aria-label="Close" class="octicon octicon-x js-menu-close" height="16" role="img" version="1.1" viewBox="0 0 12 16" width="12"><path fill-rule="evenodd" d="M7.48 8l3.75 3.75-1.48 1.48L6 9.48l-3.75 3.75-1.48-1.48L4.52 8 .77 4.25l1.48-1.48L6 6.52l3.75-3.75 1.48 1.48z"></path></svg></a>
																	</div>
																	<div class="fenetre-body">
																		<form action="index.php?modifierModuleSub" method="post">
																			<input type="hidden" name="UE_ID" value="<?php echo $ue['ID']; ?>"/>
																			<input type="hidden" name="ID" value="<?php echo $module['ID']; ?>"/>
																			Nom : <input type="text" name="nom" value="<?php echo $module['nom']; ?>" required /><br/><br />
															        Nombre d'heures TDs : <input type="number" name="nbHeuresTD" placeholder="TD" value="<?php echo $module['nbHeuresTD']; ?>" required></br></br>
															        Nombre d'heures TPs : <input type="number" name="nbHeuresTP" placeholder="TP" value="<?php echo $module['nbHeuresTP']; ?>" required></br></br>
															        Nombre d'heures CMs : <input type="number" name="nbHeuresCM" placeholder="CM" value="<?php echo $module['nbHeuresCM']; ?>" required></br></br>
															        Nombre d'heures DS : <input type="number" name="nbHeuresDS" placeholder="DS"  value="<?php echo $module['nbHeuresDS']; ?>" required></br></br>
															        Numéro de la période de départ : <input type="text" name="periode" placeholder="1 à 4" min="1" max="4" value="<?php echo $module['periode']; ?>" required></br></br>
																			Durée du module (semaines) : <input type="number" name="duree" value="<?php echo $module['duree'] ?>" required><br><br>
				...
				<!-- Méthode utilisée après inscription, lorsque l'utilisateur n'a encore pas choisi de formation -->

				<div class="vide">
						<p class="errorMessage">Vous n'êtes inscrit dans aucun département</p>
						<a href="?rejoindreFormation"><button>Rejoindre un département</button><a>
						<span>
							<a href="?ajoutFormation"><button class="boutonAjouter">Créer un département</button></a>
							<div class="fenetreModale">
								<div class="fenetre-header">
									Nouveau département<a class="close"><svg aria-label="Close" class="octicon octicon-x js-menu-close" height="16" role="img" version="1.1" viewBox="0 0 12 16" width="12"><path fill-rule="evenodd" d="M7.48 8l3.75 3.75-1.48 1.48L6 9.48l-3.75 3.75-1.48-1.48L4.52 8 .77 4.25l1.48-1.48L6 6.52l3.75-3.75 1.48 1.48z"></path></svg></a>
								</div>
								<div class="fenetre-body">
							      ...
							      <?php if (isset($_GET["erreur"])) {
							        switch ($_GET["erreur"]) {
							          case '1':
							            echo "Veuillez entre un nom pour le département";
							            break;

							          case '2':
							            echo "Si vous n'êtes pas administrateur du département, veuillez entre l'adresse mail d'un administrateur";
							            break;
							          case '3':
							      ...
							      <p style="max-width:400px;">Attention ! Assurez-vous d'avoir l'accord du chef <br/> du département avant d'ajouter le département.</p>
							        ...
							        <button id="boutonNiveau" type="hidden"> Ajouter un niveau</button>
